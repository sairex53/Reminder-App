...
struct PinCode<Content: View>: View {
                    ...
                    if (lockType == .both && !noBiometricAccess) || lockType == .biometric {
                        Group {
                            if noBiometricAccess {
                                Text("Unable biometric authentication in Settings to unlock the view.")
                                    .font(.callout)
    ...
    private func NumberPadPinView() -> some View {
        VStack(spacing: 180) {
            ...
            .keyframeAnimator(initialValue: CGFloat.zero, trigger: animateField, content:
                { content, value in
    ...
    enum LockType: String {
        case biometric = "NoneBiometric"
        case number = "NoneNumber"
        case both = "NoneBoth"
    }
}
